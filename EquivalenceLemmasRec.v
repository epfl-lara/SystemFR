...
Lemma equivalent_rec_succ:
  forall n T e1 e2 v,
    ...
    equivalent (rec T n e1 e2) (open 0 (open 1 e2 v)
                                               (lambda T_unit (rec T v e1 e2))).
Proof.
  unfold equivalent in *; repeat step || t_invert_star || t_deterministic_star;
    eauto using star_smallstep_trans with bsteplemmas smallstep.
  eapply star_many_steps; try eassumption; eauto using value_irred;
...
Lemma equivalent_rec_succ2:
  forall n T e1 e2 v e,
    is_nat_value v ->
    star small_step n (succ v) ->
    equivalent (rec T n e1 e2) e ->
    equivalent  (open 0 (open 1 e2 v)
                                               (lambda T_unit (rec T v e1 e2))) e.
Proof.
  intros.
