...
Lemma reducible_values_rec_step:
  forall theta t1 t2 T t,
    star small_step t1 t2 ->
    reducible_values theta t (T_rec t1 T) ->
    reducible_values theta t (T_rec t2 T).
Proof.
  repeat step || simp_red;
    eauto with berased.
  - left; eapply star_many_steps; eauto; unfold irred; repeat step || t_invert_step.
...
Proof.
  repeat step || simp_red;
    eauto with berased.
  - left; eauto using star_smallstep_trans.
...
Lemma equivalent_rc_rec_step:
  forall theta t1 t2 T,
    ...
    star small_step t1 t2 ->
    equivalent_rc
      (fun t => reducible_values theta t (T_rec t1 T))
      (fun t => reducible_values theta t (T_rec t2 T)).
Proof.
...
Lemma reducible_unfold:
  forall theta t n T,
    ...
    reducible theta t (T_rec (succ n) T) ->
    reducible theta (tunfold t) (topen 0 T (T_rec n T)).
Proof.
  ...
  - apply star_smallstep_trans with (tunfold (tfold v'));
      eauto with bsteplemmas; eauto with smallstep.
  - eapply reducibility_subst_head; eauto; repeat step || t_listutils.
    ...
    rewrite is_erased_term_tfv in *; steps.
Qed.
...
Lemma open_reducible_unfold:
  forall tvars gamma t n T,
    ...
    is_erased_term n ->
    is_erased_type T ->
    open_reducible tvars gamma t (T_rec (succ n) T) ->
    open_reducible tvars gamma (tunfold t) (topen 0 T (T_rec n T)).
Proof.
...
Lemma reducible_fold:
  forall theta t n T,
    ...
    reducible theta t (topen 0 T (T_rec n T)) ->
    reducible theta (tfold t) (T_rec (succ n) T).
Proof.
  unfold reducible, reduces_to;
    repeat match goal with
           | _ => step || simp_red
  ...
  repeat step || simp_red;
    eauto with berased;
    eauto using red_is_val with values;
    eauto with bfv bwf.

  right.
  unshelve eexists t'0, t', (makeFresh (pfv n type_var :: pfv t'0 type_var :: pfv T type_var :: support theta :: nil)), _, _;
    repeat step;
    try finisher;
  ...
  | |- reducible_values ((?M,_) :: _) _ _ =>
    eapply (reducible_rename_one_rc (fun v : tree => reducible_values theta v (T_rec n T)) _ _ _ _ M M);
    repeat step || apply equivalent_rc_rec_step
  end;
    try finisher;
    eauto using reducibility_is_candidate.

  apply reducibility_subst_head2; repeat step || t_listutils; try finisher;
    eauto with btwf bwf.
Qed.
...
Lemma open_reducible_fold:
  forall tvars gamma t n T,
    ...
    open_reducible tvars gamma t (topen 0 T (T_rec n T)) ->
    open_reducible tvars gamma (tfold t) (T_rec (succ n) T).
Proof.
  unfold open_reducible;
