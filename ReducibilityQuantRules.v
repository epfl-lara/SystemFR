...
Lemma reducible_forall:
  forall t U V A,
    ...
    wf U 0 ->
    fv U = nil ->
    (forall u, reducible_values u U -> reducible t (open 0 V u)) ->
    reducible t (T_forall U V).
Proof.
...
Lemma open_reducible_forall:
  forall gamma x t U V A,
    ...
    open_reducible ((x, U) :: gamma) t (open 0 V (fvar x)) ->
    open_reducible gamma t (T_forall U V).
Proof.
  unfold open_reducible in *; repeat step || tt.
  ...
  eapply reducible_forall; steps;
    ...
    eauto with values;
    eauto with btf.
  unshelve epose proof (H7 ((x,u) :: l) _); tac1.
Qed.

Lemma open_reducible_exists_elim:
  forall (gamma : context) (p : term) U V (t : term) T u v,
    ~(u ∈ fv_context gamma) ->
    ~(u ∈ fv t) ->
    ~(u ∈ fv U) ->
    ~(u ∈ fv V) ->
    ...
    subset (fv t) (support gamma) ->
    open_reducible gamma p (T_exists U V) ->
    open_reducible ((v, open 0 V (fvar u)) :: (u, U) :: gamma) (open 0 t (fvar v)) T ->
    open_reducible gamma (tlet p (T_exists U V) t) T.
Proof.
  unfold open_reducible; repeat step || tt.
  pose proof H5 as Copy.
  unfold reducible, reduces_to in H5.
  repeat step || tt || t_listutils || simp_red || t_deterministic_star || apply reducible_let_rule;
    eauto with bwf; eauto with bfv.

  unshelve epose proof (H17 ((v,v0) :: (u,a) :: l) _); tac1.
Qed.

Lemma reducible_subtype_forall:
  forall (gamma : context) (t : term) T1 T2,
    open_reducible gamma t T1 ->
    forall (t0 : term) (l : list (nat * term)),
      satisfies reducible_values gamma l ->
      reducible_values t0 (T_forall (substitute T1 l) (substitute T2 l)) ->
      reducible_values t0 (T_let (substitute t l) (substitute T1 l) (substitute T2 l)).
Proof.
...
Lemma reducible_subtype_exists:
  forall (gamma : context) (t : term) T1 T2,
    ...
    forall (t0 : term) (l : list (nat * term)),
      satisfies reducible_values gamma l ->
      reducible_values t0 (T_let (substitute t l) (substitute T1 l) (substitute T2 l)) ->
      reducible_values t0 (T_exists (substitute T1 l) (substitute T2 l)).
Proof.
  unfold open_reducible, reducible, reduces_to;
    repeat step || tt || simp_red || t_deterministic_star; eauto.
Qed. 
