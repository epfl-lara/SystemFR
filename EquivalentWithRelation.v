...
Definition equivalent_with { T } (l1 l2: map nat T) (x y: nat) (equiv: T -> T -> Prop) :=
  (forall v1, lookup PeanoNat.Nat.eq_dec l1 x = Some v1 ->
  exists v2, lookup PeanoNat.Nat.eq_dec l2 y = Some v2  /\ equiv v1 v2) /\
  (forall v2, lookup PeanoNat.Nat.eq_dec l2 y = Some v2 ->
  exists v1, lookup PeanoNat.Nat.eq_dec l1 x = Some v1  /\ equiv v1 v2).
...
Lemma instantiate_rel:
  forall T rel ρ ρ' x y P (equiv: T -> T -> Prop),
    ...
    (exists P', equiv P P' /\
           lookup PeanoNat.Nat.eq_dec ρ' y = Some P').
Proof.
